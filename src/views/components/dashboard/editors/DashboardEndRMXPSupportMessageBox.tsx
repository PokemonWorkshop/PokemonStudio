...
import { useProjectSave } from '@root/src/hooks/useProjectSave';

type DashboardEndRMXPSupportMessageBoxState = 'select_mode' | 'save' | 'reload_project';

type DashboardEndRMXPSupportMessageBoxProps = {
  closeDialog: () => void;
};

export const DashboardEndRMXPSupportMessageBox = ({ closeDialog }: DashboardEndRMXPSupportMessageBoxProps) => {
  const loaderRef = useLoaderRef();
  const { save } = useProjectSave();
  const projectLoad = useProjectLoad();
  const [state, setState] = useState<DashboardEndRMXPSupportMessageBoxState>('select_mode');
  const [mode, setMode] = useState<'tiled' | undefined>(undefined);
  ...
  return (
    <MessageBoxContainer>
      ...
      <MessageBoxTextContainer>
        <p>{t('end_rmxp_support_message_box')}</p>
        <p className="red" style={{ marginTop: '1rem' }}>
          {t('end_rmxp_support_warning_message')}
        </p>
      </MessageBoxTextContainer>
      <MessageBoxActionContainer>
