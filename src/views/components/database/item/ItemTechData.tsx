...
export const ItemTechData = ({ dialogsRef }: ItemTechDataProps) => {
  ...
  const isClickable: boolean = useKeyPress(CONTROL) && moveName !== t('move_deleted');
    ...
    >
      {!isDisabled && (
        <DataGrid rows="1fr 1fr 1fr">
          <DataFieldsetField label={t('machines_category')} data={isItemTech ? t(`${item.isHm ? 'hm' : 'tm'}`) : '---'} disabled={!isItemTech} />
          <DataFieldsetField
            label={t('move_learnt')}
            data={moveName}
            disabled={!isItemTech}
          ...
          />
          <DataFieldsetField label={t('fling')} data={item.flingPower} />
        </DataGrid>
      )}
    </DataBlockWithTitle>
  );
