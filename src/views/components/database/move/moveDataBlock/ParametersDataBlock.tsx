...
import { MoveBattleEngineMethod } from '@modelEntities/move/Move.model';
...
export const ParametersDataBlock = ({ move, onClick }: MoveDataProps) => {
  ...
  const getCategory = () => {
    if (!(MoveBattleEngineMethod as ReadonlyArray<string>).includes(move.battleEngineMethod)) return t('database_moves:custom');
