...
const getFormWithCurrentMove = (pokemon: StudioCreature, move: StudioMove) => {
  ...
  return pokemon.forms.find((form) => form.moveSet.find((m) => m.klass === 'BreedLearnableMove' && m.move === move.dbSymbol)) || pokemon.forms[0];
};
...
const getAllPokemonWithCurrentBreedLearnableMove = (state: State, move: StudioMove) => {
  return Object.values(state.projectData.pokemon).filter((pokemon) =>
    pokemon.forms.find((form) => form.moveSet.find((m) => m.klass === 'BreedLearnableMove' && m.move === move.dbSymbol))
  );
};
