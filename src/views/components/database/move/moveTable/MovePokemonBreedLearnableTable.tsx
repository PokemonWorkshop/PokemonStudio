...
const getFormWithCurrentMove = (pokemon: PokemonModel, move: MoveModel) => {
  ...
  return pokemon.forms.find((form) => form.breedLearnableMove.find((blm) => blm.move === move.dbSymbol)) || pokemon.forms[0];
};
...
export const MovePokemonBreedLearnableTable = ({ move }: MoveDataProps) => {
  ...
  const allPokemon = move.getAllPokemonWithCurrentBreedLearnableMove(state).sort((a, b) => a.id - b.id);
