...
const getFormWithCurrentMove = (pokemon: StudioCreature, move: StudioMove) => {
  ...
  return pokemon.forms.find((form) => form.moveSet.find((m) => m.klass === 'TutorLearnableMove' && m.move === move.dbSymbol)) || pokemon.forms[0];
};
...
const getAllPokemonWithCurrentTutorLearnableMove = (state: State, move: StudioMove) => {
  return Object.values(state.projectData.pokemon).filter((pokemon) =>
    pokemon.forms.find((form) => form.moveSet.find((m) => m.klass === 'TutorLearnableMove' && m.move === move.dbSymbol))
  );
};

type MovePokemonTutorLearnableTableProps = {
  move: StudioMove;
};

export const MovePokemonTutorLearnableTable = ({ move }: MovePokemonTutorLearnableTableProps) => {
  const [state] = useGlobalState();
  const { t } = useTranslation(['database_types', 'database_moves', 'database_pokemon']);
  const allPokemon = getAllPokemonWithCurrentTutorLearnableMove(state, move).sort((a, b) => a.id - b.id);
