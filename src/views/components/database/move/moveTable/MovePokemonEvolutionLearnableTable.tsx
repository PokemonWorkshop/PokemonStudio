...
const getFormWithCurrentMove = (pokemon: StudioCreature, move: StudioMove) => {
  ...
  return pokemon.forms.find((form) => form.moveSet.find((m) => m.klass === 'EvolutionLearnableMove' && m.move === move.dbSymbol)) || pokemon.forms[0];
};
 ...
 */
const getAllPokemonWithCurrentEvolutionLearnableMove = (state: State, move: StudioMove) => {
  return Object.values(state.projectData.pokemon).filter((pokemon) =>
    pokemon.forms.find((form) => form.moveSet.find((m) => m.klass === 'EvolutionLearnableMove' && m.move === move.dbSymbol))
  );
};

type MovePokemonEvolutionLearnableTableProps = {
  move: StudioMove;
};

export const MovePokemonEvolutionLearnableTable = ({ move }: MovePokemonEvolutionLearnableTableProps) => {
  const [state] = useGlobalState();
  const { t } = useTranslation(['database_types', 'database_moves', 'database_pokemon']);
  const allPokemon = getAllPokemonWithCurrentEvolutionLearnableMove(state, move).sort((a, b) => a.id - b.id);
