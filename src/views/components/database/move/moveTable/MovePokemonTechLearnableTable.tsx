...
const getFormWithCurrentMove = (pokemon: PokemonModel, move: MoveModel) => {
  ...
  return pokemon.forms.find((form) => form.techLearnableMove.find((tlm) => tlm.move === move.dbSymbol)) || pokemon.forms[0];
};
...
export const MovePokemonTechLearnableTable = ({ move }: MoveDataProps) => {
  ...
  const allPokemon = move.getAllPokemonWithCurrentTechLearnableMove(state).sort((a, b) => a.id - b.id);
