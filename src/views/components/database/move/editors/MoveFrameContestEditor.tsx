...
import { MOVE_CONDITIONS, MOVE_CONTEST_DATA_VALIDATOR, MOVE_VALIDATOR } from '@modelEntities/move';
...
const FRAME_EDITOR_SCHEMA = MOVE_CONTEST_DATA_VALIDATOR.pick({ condition: true });
...
export const MoveFrameContestEditor = forwardRef<EditorHandlingClose>((_, ref) => {
  ...
  const { canClose, getFormData, defaults, formRef } = useZodForm(FRAME_EDITOR_SCHEMA, move.contestData);
  ...
  return (
    <Editor type="edit" title={t('information')}>
      <InputFormContainer ref={formRef}>
        ...
        <Select name="condition" label={t('condition')} options={conditionOptions} />
      </InputFormContainer>
