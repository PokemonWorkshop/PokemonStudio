  ...
  return (
    <DataBlockWithTitle size="full" title={t('musics')} disabled={disabled} onClick={() => dialogsRef.current?.openDialog('musics')}>
      <DataGrid columns="1fr 1fr 1fr" rows="1fr 1fr">
        ...
        <DataFieldsetField label={'Volume'} data={`${map.bgm.volume}%`} disabled={!map.bgm.name} />
        <DataFieldsetField label={'Volume'} data={`${map.bgs.volume}%`} disabled={!map.bgs.name} />
        <DataFieldsetField label={'Tempo'} data={`${map.bgm.pitch}%`} disabled={!map.bgm.name} />

        <DataFieldsetField label={'Pitch'} data={`${map.bgs.pitch}%`} disabled={!map.bgs.name} />
      </DataGrid>
    </DataBlockWithTitle>
  );
};
