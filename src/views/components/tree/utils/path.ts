 ...
 */
export const hasSameParent = (a: Path, b: Path): boolean => isSamePath(getParentPath(a), getParentPath(b));
...
*/
export const isTopOfSubtree = (belowPath: Path, abovePath?: Path) => !abovePath || isParentOf(abovePath, belowPath);
...
export const isLowerSibling = (a: Path, other: Path): boolean => hasSameParent(a, other) && getIndexAmongSiblings(a) > getIndexAmongSiblings(other);
