...
export const getFlatItemPath = (flattenedTree: FlattenedTree, sourceIndex: number): Path => flattenedTree[sourceIndex].path;
...
export const getDestinationPath = (
  ...
  const lowerPath: Path =
    down || sameIndex ? flattenedTree[destinationIndex + 1] && flattenedTree[destinationIndex + 1].path : flattenedTree[destinationIndex].path;
...
const calculateFinalLevel = (sourcePath: Path, upperPath: Path, lowerPath?: Path, level?: number): number => {
...
export const getItemById = (flattenedTree: FlattenedTree, id: ItemId): FlattenedItem | undefined => flattenedTree.find((item) => item.item.id === id);

export const getIndexById = (flattenedTree: FlattenedTree, id: ItemId): number => flattenedTree.findIndex((item) => item.item.id === id);
