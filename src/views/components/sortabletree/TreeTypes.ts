...
export type TreeItems<T extends Record<string, unknown>> = TreeItem<T>[];
export type TreeItemComponentProps<T = Record<string, never>> = {
  item: TreeItem<T>;
...
export type SortableTreeLimitationType<T> = (activeItem: FlattenedItem<T> | null | undefined, projected: ProjectionItem<T>) => boolean;
