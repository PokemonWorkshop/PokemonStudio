...
const projectStudioFile = async (event: IpcMainEvent, payload: { path: string; action: ProjectStudioAction; data?: string }) => {
  console.info("project-studio-file", { path: payload.path, action: payload.action });
  ...
  try {
    switch (payload.action) {
      case "READ": {
        ...
        console.info("project-studio-file/success");
      ...
      case "UPDATE":
        if (payload.data) {
          ...
          console.error("project-studio-file/failure", "data are missing");
    ...
    console.error("project-studio-file/failure", error);
