...
const configureNewProject = (event: IpcMainEvent, payload: { projectDirName: string; metaData: ConfigureNewProjectMetaData }) => {
  console.info("configure-new-project", payload);
  try {
    console.info("configure-new-project/create project.studio file");
    writeFileSync(path.join(payload.projectDirName, "project.studio"), payload.metaData.projectStudioData);
    console.info("configure-new-project/create psdk.bat file");
    writeFileSync(path.join(payload.projectDirName, "psdk.bat"), generatePSDKBatFileContent());
    console.info("configure-new-project/update icon");
    copyFileSync(
    ...
    if (!payload.metaData.multiLanguage) {
      console.info("configure-new-project/update game options config");
    ...
    console.error("configure-new-project/failure", error);
