...
export const MultiSelect = <Value extends ValueType, ChooseValue extends Value>(props: MultiSelectProps<Value, ChooseValue>) => {
  const { currentValues, onSelectValue, optionsUtilsRef, popoverRef, inputRef, outputRef, listRef, inputProps, outputProps } = useMultiSelect({...props, chooseValue: props.chooseValue as Value[] ?? []});

  return (
    <MultiSelectContainer className={props.className}>
