...
expect.extend({
  toKeepIntegrity(
    ...
    received: any,
    model: Serializable<PSDKEntity>
  ): jest.CustomMatcherResult {
    const serializer = new TypedJSON(model);
    // eslint-disable-next-line jest/no-standalone-expect
    const pass =
      diff(serializer.toPlainJson(serializer.parse(received)!), received) ===
      undefined;
    const message = () =>
      `Expected object ${JSON.stringify(received)} does ${
        this.isNot ? '' : 'not'
      } keep integrity. Diff: ${diffString(
