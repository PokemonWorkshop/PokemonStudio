...
/**
 * Run Studio's map-update pass (tileMetadata + overviews) straight after saving,
 * so the map's cached metadata and the "modified" dot stay in step without a
 * second click.
 *
 * NOTE: this does NOT write Data/Map###.rxdata. Nothing in Studio does — the
 * only backends touching those files read them, or write events. The tile data
 * .rxdata is generated by PSDK's Tiled2Rxdata when the game boots, which is why
 * a map change needs a game launch before it takes effect.
 */
const AUTO_CONVERT_KEY = 'pokemonstudio.fork.mapEditor.autoRefreshMapMetadata';
const loadAutoConvert = (): boolean => {
  try { return localStorage.getItem(AUTO_CONVERT_KEY) !== 'false'; }
  catch { return true; }
};
const saveAutoConvert = (value: boolean) => {
  try { localStorage.setItem(AUTO_CONVERT_KEY, String(value)); }
