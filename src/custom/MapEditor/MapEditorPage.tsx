...
export const MapEditorPage = () => {
  ...
  const mapEvents = useMapEvents(
    ...
    map?.tiledFilename,
    // Adopt event edits parked when we last left this map, instead of re-reading
    // the .rxdata and throwing them away.
    useCallback((id: number) => {
      const entry = Object.values(studioMapsRef.current).find((m) => m.id === id);
      return entry ? getPendingEdit(entry.dbSymbol)?.events : undefined;
    }, []),
  );
  ...
  // The canvas calls onBeforeTeardown from an effect cleanup, so the callback
  // must read dirty through a ref rather than a stale closure.
  const dirtyRef = useRef(false);
  dirtyRef.current = dirty;
  // Read inside the canvas teardown callback, which runs outside render.
  const studioMapsRef = useRef(studioMaps);
  studioMapsRef.current = studioMaps;
  // Maps parked by earlier visits — they count towards "Save maps" even when
  // the map on screen right now is clean.
  ...
  const refreshStudioCacheFor = useCallback((dbSymbol: string, tiledFilename: string) => new Promise<void>((resolve) => {
    ...
    window.api.convertTiledMapToTileMetadata(
      { tmxPath },
      (payload) => {
        const current = studioMapsRef.current[dbSymbol];
        if (!current) return;
        ...
        setStudioMap({ [dbSymbol]: { ...current, ...payload, sha1: payload.sha1 as Sha1 } });
        setGlobalState((s) => ({
          ...s,
  ...
  const onSave = useCallback(async () => {
    ...
    try {
      await saveBytes(projectPath!, map.tiledFilename!, bytes);
      // Written to disk — nothing left to park for this map.
      if (map?.dbSymbol) clearPendingEdit(map.dbSymbol, 'tiles');
      setDirty(false);
      cleanHistoryDepthRef.current = historyPastRef.current.length;
      bumpHistory();
   ...
   *
   * Keyed off `mapEvents.loadedMapId` — the map the hook ACTUALLY has in memory
   * — never off `map`, which React has already advanced to the incoming map
   * during a switch. That mismatch is what filed one map's tiles onto another.
   *
   * Parking here (rather than on teardown) is safe and cheap: events are plain
   * objects, so there's no serialization cost, and it means the map tree's dot
   * lights up the moment you edit an event rather than when you navigate away.
   * Skipped events are never parked — the hook refuses to write a partially-read
   * map, so parking it would only create an entry that can never be saved.
   */
  useEffect(() => {
    const loadedId = mapEvents.loadedMapId;
    if (loadedId === null || !mapEvents.dirty || mapEvents.skipped > 0) return;
    const owner = Object.values(studioMapsRef.current).find((m) => m.id === loadedId);
    if (!owner) return;
    setPendingEdit({
      dbSymbol: owner.dbSymbol,
      mapName: owner.dbSymbol,
      tiledFilename: owner.tiledFilename,
      events: mapEvents.events,
    });
  }, [mapEvents.loadedMapId, mapEvents.dirty, mapEvents.skipped, mapEvents.events]);

  /**
   * Write the map on screen (if dirty) AND every map parked from an earlier
   * visit. Without the parked pass, "Save maps" could only ever write the map
  ...
  /**
   * Write the open map's events (if dirty) AND every map's events parked from an
   * earlier visit. Kept separate from onSaveAllMaps so "Save maps" and "Save
   * events" mean exactly what they say — folding them together made one menu
   * entry quietly do the other's job.
   */
  const onSaveAllEvents = useCallback(async (only?: Set<string>) => {
    const live = mapEventsRef.current;
    const liveId = live.loadedMapId;
    // The live map goes through the hook so its dirty flag actually resets.
    const wanted = (dbSymbol?: string) => !only || (!!dbSymbol && only.has(dbSymbol));
    const liveOwner = liveId === null ? undefined : Object.values(studioMapsRef.current).find((m) => m.id === liveId);
    if (live.dirty && wanted(liveOwner?.dbSymbol)) {
      // Drop the park BEFORE saving, not after. save() reloads on success to
      // re-point provenance at the freshly written file, and reload adopts any
      // parked events — so a park still sitting in the store would come straight
      // back in and mark the map dirty again the instant we saved it.
      //
      // Safe to drop early: if the write fails the hook stays dirty, and the
      // eager-park effect immediately re-parks the same events.
      if (liveOwner) clearPendingEdit(liveOwner.dbSymbol, 'events');
      setEventsSaveError(await live.save());
    }
    for (const edit of getPendingEdits()) {
      if (!edit.events || !wanted(edit.dbSymbol)) continue;
      const owner = studioMapsRef.current[edit.dbSymbol];
      // Skip the live map — handled above, and writing it twice would be wasteful.
      if (!owner || owner.id === liveId) continue;
      await new Promise<void>((resolve) => {
        window.api.writeRMXPEvents(
          // Same conversion the live map uses — see buildEventsWritePayload.
          { projectPath: projectPath!, mapId: owner.id, events: buildEventsWritePayload(edit.events!), skippedOnRead: 0 },
          () => { clearPendingEdit(edit.dbSymbol, 'events'); resolve(); },
          ({ errorMessage }) => { console.error('[events] saving parked events failed', edit.dbSymbol, errorMessage); resolve(); },
        );
      });
    }
  }, [projectPath]);

  // Publish this map's two save actions so the app-shell save button can offer
  // them in its split menu. Cleared on unmount, which greys the entries out
  ...
  useEffect(() => {
    ...
    return () => setMapEditorSaveTargets(null);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [onSaveAllMaps, onSaveAllEvents, dirty, mapEvents.dirty, pendingEdits]);

  /**
   * Rows for the save dialog: every parked map PLUS the map on screen.
   *
   * The open map's unsaved tiles live in the canvas, not in the pending store —
   * tiles are only parked when you switch away — so reading the store alone
   * showed an empty dialog while the toolbar said "unsaved changes". Its events
   * DO sit in the store (they're parked eagerly), hence the union rather than a
   * simple append.
   */
  const saveDialogRows = useMemo<SaveMapRow[]>(() => {
    const rows = new Map<string, SaveMapRow>();
    pendingEdits.forEach((edit) => rows.set(edit.dbSymbol, { dbSymbol: edit.dbSymbol, hasTiles: !!edit.tiles, hasEvents: !!edit.events }));
    if (map?.dbSymbol) {
      const existing = rows.get(map.dbSymbol);
      const hasTiles = dirty || !!existing?.hasTiles;
      const hasEvents = mapEvents.dirty || !!existing?.hasEvents;
      if (hasTiles || hasEvents) rows.set(map.dbSymbol, { dbSymbol: map.dbSymbol, hasTiles, hasEvents });
    }
    return [...rows.values()];
  }, [pendingEdits, map?.dbSymbol, dirty, mapEvents.dirty]);

  /** Write exactly what the dialog's checkboxes asked for. */
  const onConfirmSaveSelection = useCallback(
    async (selection: SaveSelection[]) => {
      setSaveDialogOpen(false);
      const tiles = new Set(selection.filter((row) => row.tiles).map((row) => row.dbSymbol));
      const events = new Set(selection.filter((row) => row.events).map((row) => row.dbSymbol));
      // Events BEFORE tiles, deliberately. writeRMXPEvents rewrites
      // Data/Map###.rxdata, so saving events stamps that file newer than the
      // .tmx — and PSDK's Tiled2Rxdata skips a map whose .rxdata is newer than
      // its source, silently dropping the tile edits. Writing the .tmx last
    ...
    <PageStyle>
      ...
      {saveDialogOpen && (
        <SaveMapsDialog
          ...
          onConfirm={(selection) => void onConfirmSaveSelection(selection)}
        />
      )}
      {canRender ? (
        <Workspace
        ...
        >
          ...
          <PixiMapCanvas
            ...
            reloadKey={reloadKey}
            pendingBytes={map?.dbSymbol ? getPendingEdit(map.dbSymbol)?.tiles ?? null : null}
            onRestoredPending={() => setDirty(true)}
            onBeforeTeardown={(loadedTiledFilename, serialize) => {
              // Park the outgoing map's unsaved tiles so switching maps doesn't
              // throw them away (RPG Maker XP behaviour).
              //
              // The identity MUST come from the canvas, not from `map`: React has
              // already re-rendered this component with the INCOMING map by the
              // time the canvas tears the old one down. Keying off `map` here
              // filed one map's tiles under the next map's db_symbol.
              if (!dirtyRef.current) return;
              const outgoing = Object.values(studioMapsRef.current).find((m) => m.tiledFilename === loadedTiledFilename);
              if (!outgoing) return;
              const bytes = serialize();
              if (!bytes) return;
              setPendingEdit({
                dbSymbol: outgoing.dbSymbol,
                mapName: outgoing.dbSymbol,
                tiledFilename: outgoing.tiledFilename,
                tiles: bytes,
              });
            }}
            onLoaded={onLoaded}
